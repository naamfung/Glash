...
func hyperBuilder(model string) builderFunc {
	return func(t *testing.T, r *vcr.Recorder) (fantasy.LanguageModel, error) {
		provider, err := openaicompat.New(
			...
			openaicompat.WithAPIKey(os.Getenv("GLASH_HYPER_API_KEY")),
...
func testEnv(t *testing.T) fakeEnv {
	workingDir := filepath.Join("/tmp/glash-test/", t.Name())
...
func coderAgent(r *vcr.Recorder, env fakeEnv, large, small fantasy.LanguageModel) (SessionAgent, error) {
	...
	// independently of user config on `$HOME/.config/glash/glash.json`.
	...
	cfg.Config().Options.DisabledSkills = []string{"glash-config"}
	...
	allTools := []fantasy.AgentTool{
		tools.NewShellTool(env.permissions, env.workingDir, cfg.Config().Options.Attribution, modelName),
