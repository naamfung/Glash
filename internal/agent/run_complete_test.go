...
// event into a closure nobody reads, and subscribers (`glash run`)
...
// publish a RunComplete for its RunID, hanging a `glash run` caller that
...
// than internal bookkeeping, which is the contract a `glash run` caller
...
// bookkeeping slice, not the published event a `glash run` caller awaits.
