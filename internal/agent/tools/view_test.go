...
func TestReadBuiltinFile(t *testing.T) {
	...
	t.Run("reads glash-config skill", func(t *testing.T) {
		t.Parallel()

		resp, err := readBuiltinFile(ViewParams{
			FilePath: "glash://skills/glash-config/SKILL.md",
		}, nil)
		require.NoError(t, err)
		require.NotEmpty(t, resp.Content)
		require.Contains(t, resp.Content, "Glash Configuration")
	})
	...
	t.Run("not found", func(t *testing.T) {
		...
		resp, err := readBuiltinFile(ViewParams{
			FilePath: "glash://skills/nonexistent/SKILL.md",
		}, nil)
		require.NoError(t, err)
		require.True(t, resp.IsError)
	})
	...
	t.Run("metadata has skill info", func(t *testing.T) {
		...
		require.Equal(t, "glash-config", meta.ResourceName)
