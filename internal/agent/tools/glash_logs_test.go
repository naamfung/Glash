...
func createTestLogFile(t *testing.T, entries []map[string]any) string {
	t.Helper()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "glash.log")
...
func TestNewGlashLogsTool(t *testing.T) {
	t.Parallel()
	tool := NewGlashLogsTool("/tmp/test.log")
	require.NotNil(t, tool)
	require.Equal(t, GlashLogsToolName, tool.Info().Name)
}

func TestGlashLogs_HappyPath(t *testing.T) {
	t.Parallel()
	...
	result := runGlashLogs(logFile, GlashLogsParams{Lines: 3})
...
func TestGlashLogs_DefaultLines(t *testing.T) {
	t.Parallel()
	...
	result := runGlashLogs(logFile, GlashLogsParams{Lines: 0})
...
func TestGlashLogs_MaxCap(t *testing.T) {
	t.Parallel()
	...
	result := runGlashLogs(logFile, GlashLogsParams{Lines: 200})

	lines := strings.Split(result, "\n")
	require.Len(t, lines, 100)
}

func TestGlashLogs_MissingFile(t *testing.T) {
	t.Parallel()
	result := runGlashLogs("/nonexistent/path/glash.log", GlashLogsParams{Lines: 50})
	require.Contains(t, result, "No log file found")
}

func TestGlashLogs_EmptyFile(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "glash.log")
	...
	result := runGlashLogs(logFile, GlashLogsParams{Lines: 50})
	require.Contains(t, result, "Log file is empty")
}

func TestGlashLogs_MalformedLines(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "glash.log")
	...
	result := runGlashLogs(logFile, GlashLogsParams{Lines: 10})
...
func TestGlashLogs_ExtraFieldsSorted(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
	...
	result := runGlashLogs(logFile, GlashLogsParams{Lines: 1})
...
func TestGlashLogs_NonStringValues(t *testing.T) {
	t.Parallel()
	entry := map[string]any{
...
func TestGlashLogs_Redaction(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
...
func TestGlashLogs_ReservedFields(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
		{
			"time":   time.Now().Format(time.RFC3339),
...
func TestGlashLogs_OversizedLines(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "glash.log")
...
func TestGlashLogs_PartialTrailingLine(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "glash.log")
...
func TestGlashLogs_ValueQuoting(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
...
func TestGlashLogs_ChronologicalOrder(t *testing.T) {
	t.Parallel()
...
func TestGlashLogs_TimeOnlyFormat(t *testing.T) {
	t.Parallel()
	entry := map[string]any{
...
func TestGlashLogs_LevelVariations(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
	...
	result := runGlashLogs(logFile, GlashLogsParams{Lines: 5})
...
func TestGlashLogs_SourceVariations(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
