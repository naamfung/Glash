...
const GlashLogsToolName = "glash_logs"

//go:embed glash_logs.md.tpl
var glashLogsDescriptionTmpl []byte

var glashLogsDescriptionTpl = template.Must(
	template.New("glashLogsDescription").
		Parse(string(glashLogsDescriptionTmpl)),
)

type glashLogsDescriptionData struct {
	DefaultLines int
	MaxLines     int
}

func glashLogsDescription() string {
	return renderTemplate(glashLogsDescriptionTpl, glashLogsDescriptionData{
...
type GlashLogsParams struct {
	Lines int `json:"lines,omitempty" description:"Number of recent log entries to return (default 50, max 100)"`
}

func NewGlashLogsTool(logFile string) fantasy.AgentTool {
	return fantasy.NewAgentTool(
		GlashLogsToolName,
		glashLogsDescription(),
		func(ctx context.Context, params GlashLogsParams, call fantasy.ToolCall) (fantasy.ToolResponse, error) {
			result := runGlashLogs(logFile, params)
			return fantasy.NewTextResponse(result), nil
		},
	)
}

// runGlashLogs reads and formats the last N log entries from the given file.
func runGlashLogs(logFile string, params GlashLogsParams) string {
