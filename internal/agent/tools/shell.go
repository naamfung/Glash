...
func shellDescription(attribution *config.Attribution, modelID string) string {
...
func NewShellTool(permissions permission.Service, workingDir string, attribution *config.Attribution, modelID string) fantasy.AgentTool {
	return fantasy.NewAgentTool(
		ShellToolName,
		string(shellDescription(attribution, modelID)),
		func(ctx context.Context, params ShellParams, call fantasy.ToolCall) (fantasy.ToolResponse, error) {
			...
			if !isSafeReadOnly {
				p, err := permissions.Request(
					...
					permission.CreatePermissionRequest{
						...
						Params:      ShellPermissionsParams(params),
					},
				)
				if err != nil {
			...
			if params.RunInBackground {
				...
				if done {
					...
					if stdout == "" {
						return fantasy.WithResponseMetadata(fantasy.NewTextResponse(ShellNoOutput), metadata), nil
					}
