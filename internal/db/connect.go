...
// glash process from racing the storage.
...
// workspace-bootstrap path opts in. GLASH_SKIP_DATADIR_LOCK still
...
func Connect(ctx context.Context, dataDir string, opts ...ConnectOption) (*sql.DB, error) {
	...
	dbPath := filepath.Join(dataDir, "glash.db")
	...
	// glash process on the same SQLite file. The lock is released when
	...
	// local-mode invocations do not refuse a second glash against the
