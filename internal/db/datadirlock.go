...
// already in use by another glash process.
var ErrDataDirLocked = errors.New("data directory already in use by another glash process")

// dataDirLockFile is the name of the lock file inside the data
// directory. It lives next to glash.db so users can `ls` and find it.
const dataDirLockFile = "glash.lock"
...
// {dataDir}/glash.lock. If the lock is already held by another
...
// GLASH_SKIP_DATADIR_LOCK is set to a truthy value. This is intended
...
func skipDataDirLock() bool {
	v, _ := strconv.ParseBool(os.Getenv("GLASH_SKIP_DATADIR_LOCK"))
	return v
}
