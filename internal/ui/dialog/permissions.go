...
func (p *Permissions) renderHeader(contentWidth int) string {
	...
	case tools.ShellToolName:
		if params, ok := p.permission.Params.(tools.ShellPermissionsParams); ok {
...
func (p *Permissions) renderContent(width int) string {
	switch p.permission.ToolName {
	case tools.ShellToolName:
		return p.renderShellContent(width)
...
func (p *Permissions) renderShellContent(width int) string {
	params, ok := p.permission.Params.(tools.ShellPermissionsParams)
	if !ok {
		return ""
	}
