...
func TestPermissionRequestParamsTypeAssertable(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			params: tools.ShellPermissionsParams{
			...
			assert: func(t *testing.T, got any) {
				v, ok := got.(tools.ShellPermissionsParams)
				require.True(t, ok, "params must decode as tools.ShellPermissionsParams, got %T", got)
