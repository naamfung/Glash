...
// are what non-interactive clients (e.g. `glash run`) rely on to
...
// through the SSE envelope. Without these correlators, `glash run`
...
// "agent failed" (returns non-zero from `glash run`) from "agent
