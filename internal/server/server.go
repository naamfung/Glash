...
// socketDir returns the directory used for the Glash Unix socket.
...
// returned by [socketDir] and is named glash-<uid>.sock, falling
// back to glash.sock when the current uid cannot be determined. If
// the composed path would exceed [maxUnixSocketPathLen] bytes (the
// macOS sun_path limit), we fall back to /tmp/glash-<uid>.sock so
// the socket remains bindable.
func DefaultHost() string {
	sock := "glash.sock"
	usr, err := user.Current()
	if err == nil && usr.Uid != "" {
		sock = fmt.Sprintf("glash-%s.sock", usr.Uid)
	}
...
// Server represents a Glash server bound to a specific address.
type Server struct {
