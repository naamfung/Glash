...
// DefaultSessionName is the fallback title shown in the sidebar when a session
// has no meaningful title yet. It is not used as the stored session name in
// the Sessions list; stored session names use the Session-YYYY-MM-DD-HH-MM-SS
// format generated by GenerateSessionTitle.
const DefaultSessionName = "Current Session"

// GenerateSessionTitle returns a session title in the format "Session-YYYY-MM-DD-HH-MM-SS".
func GenerateSessionTitle() string {
	return "Session-" + time.Now().Format("2006-01-02-15-04-05")
}
