...
// GlashEnvMarkers returns a fresh slice of the environment variables that
// Glash unconditionally sets on every shell it spawns — both the interactive
...
func NewShell(opts *Options) *Shell {
	...
	// Allow tools to detect execution by Glash.
	env = append(env, GlashEnvMarkers()...)
