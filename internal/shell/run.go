...
type RunOptions struct {
	...
	// the Glash process cwd — hooks and the shell tool have different
...
// Glash handler stack. Shared by the stateless [Run] entrypoint and the
...
// standard Glash middleware chain (builtins, script dispatch, block list)
...
// when sourcing framework files can send SIGINT/SIGTERM to Glash's process
...
// regardless of the caller's environment because Glash shells are never
...
//  1. builtins first (so Glash's in-process jq wins over any PATH binary);
...
// builtinHandler returns middleware that dispatches recognized Glash
