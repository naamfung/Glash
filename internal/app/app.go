...
import (
	...
	"github.com/charmbracelet/glash/internal/update"
...
type App struct {
	...
	// subscribers (notably `glash run` in client/server mode) can
...
// for `glash run`. A lossy fan-in here can drop the only terminal
...
func shouldCheckForUpdates() bool {
	if v, _ := strconv.ParseBool(os.Getenv("GLASH_ENABLE_UPDATE_CHECK")); v {
		return true
	}
	return false
}
