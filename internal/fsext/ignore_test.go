...
func TestGlashIgnore(t *testing.T) {
	tempDir := t.TempDir()
	t.Chdir(tempDir)
	...
	// Create a .glashignore file that ignores .log files
	require.NoError(t, os.WriteFile(".glashignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .glashignore file
	glashignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".glashignore"), []byte(glashignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .glashignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .glashignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .glashignore in subdir that ignores normal_nested
	subGlashignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".glashignore"), []byte(subGlashignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .glashignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .glashignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .glashignore")
