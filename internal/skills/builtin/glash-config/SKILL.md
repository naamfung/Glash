---
name: glash-config
description: Use when the user needs help configuring Glash — working with glash.json, setting up providers, configuring LSPs, adding MCP servers, managing skills or permissions, or changing Glash behavior.
---

# Glash Configuration

Glash uses JSON configuration files with the following priority (highest to lowest):

1. `.glash.json` (project-local, hidden)
2. `glash.json` (project-local)
3. `$XDG_CONFIG_HOME/glash/glash.json` or `$HOME/.config/glash/glash.json` (global)

## Basic Structure

```json
{
  "$schema": "https://charm.land/glash.json",
  "models": {},
  "providers": {},
  "mcp": {},
  "lsp": {},
...
Glash runs selected string fields through an embedded bash-compatible
...
`glash.json` is trusted code. Any `$(...)` in it runs at load time
with the invoking user's shell privileges, before the UI appears.
Don't launch Glash in a directory whose `glash.json` you haven't
reviewed.
...
> `.agents/skills`, `.glash/skills`, `.claude/skills`, `.cursor/skills`
...
| `GLASH_TOOL_NAME` | Name of the tool being called |
| `GLASH_SESSION_ID` | Current session ID |
| `GLASH_CWD` | Current working directory |
| `GLASH_PROJECT_DIR` | Project root directory |
| `GLASH_TOOL_INPUT_COMMAND` | Value of `command` from tool input (if present) |
| `GLASH_TOOL_INPUT_FILE_PATH` | Value of `file_path` from tool input (if present) |
...
Glash also supports the Claude Code hook output format:
...
- `GLASH_GLOBAL_CONFIG` - Override global config location
- `GLASH_GLOBAL_DATA` - Override data directory location
- `GLASH_SKILLS_DIR` - Override default skills directory
