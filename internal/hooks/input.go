...
func BuildEnv(eventName, toolName, sessionID, cwd, projectDir, toolInputJSON string) []string {
	env := os.Environ()
	env = append(env, shell.GlashEnvMarkers()...)
	env = append(
		env,
		fmt.Sprintf("GLASH_EVENT=%s", eventName),
		fmt.Sprintf("GLASH_TOOL_NAME=%s", toolName),
		fmt.Sprintf("GLASH_SESSION_ID=%s", sessionID),
		fmt.Sprintf("GLASH_CWD=%s", cwd),
		fmt.Sprintf("GLASH_PROJECT_DIR=%s", projectDir),
	)

	// Extract tool-specific env vars from the JSON input.
	if toolInputJSON != "" {
		if cmd := gjson.Get(toolInputJSON, "command"); cmd.Exists() {
			env = append(env, fmt.Sprintf("GLASH_TOOL_INPUT_COMMAND=%s", cmd.String()))
		}
		if fp := gjson.Get(toolInputJSON, "file_path"); fp.Exists() {
			env = append(env, fmt.Sprintf("GLASH_TOOL_INPUT_FILE_PATH=%s", fp.String()))
		}
	}
...
// Supports both Glash format and Claude Code format (hookSpecificOutput).
