...
// with reason=tool_use and used to terminate `glash run` early via
...
// subscribers (e.g. a TUI watching workspace events while `glash
...
// non-nil error from `glash run` so shells and CI catch it via
// exit status.
...
// run (e.g. Ctrl+C while `glash run` waits) exits cleanly rather
...
// queue scenario: `glash run --continue` attaches to a session
