...
import (
	...
	"github.com/charmbracelet/glash/internal/format"
...
curl https://charm.land | glash run "Summarize this website"

# Read from a file
glash run "What is this code doing?" <<< prrr.go

# Redirect output to a file
glash run "Generate a hot README for this project" > MY_HOT_README.md

# Run in quiet mode (hide the spinner)
glash run --quiet "Generate a README for this project"

# Run in verbose mode (show logs)
glash run --verbose "Generate a README for this project"

# Continue a previous session
glash run --session {session-id} "Follow up on your last response"

# Continue the most recent session
glash run --continue "Follow up on your last response"
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if useClientServer() {
			...
			if !ws.Config.IsConfigured() {
				return fmt.Errorf("no providers configured - please run 'glash' to set up a provider interactively")
			}
...
// streaming SSE events into a final, complete stdout for `glash run`.
...
func (s *runStream) handle(ev any, stopSpinner func()) (done bool, err error) {
	...
	case pubsub.Event[proto.RunComplete]:
		...
		// source of the regression where `glash run` exited
...
// prefix returned by glash session list.
