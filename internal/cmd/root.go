...
import (
	...
	ui "github.com/charmbracelet/glash/internal/ui/model"
...
func init() {
	...
	rootCmd.PersistentFlags().StringP("data-dir", "D", "", "Custom glash data directory")
	rootCmd.PersistentFlags().BoolP("debug", "d", false, "Debug")
	rootCmd.PersistentFlags().StringVarP(&clientHost, "host", "H", server.DefaultHost(), "Connect to a specific glash server host (for advanced users)")
...
glash run "Guess my 5 favorite Pokémon"

# Run a non-interactively with pipes and redirection
cat README.md | glash run "make this more glamorous" > GLAMOROUS_README.md

# Run with debug logging in a specific directory
glash --debug --cwd /path/to/project

# Run in yolo mode (auto-accept all permissions; use with care)
glash --yolo

# Run with custom data directory
glash --data-dir /path/to/custom/.glash

# Continue a previous session
glash --session {session-id}
  ...
  `,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if _, err := program.Run(); err != nil {
			...
			return errors.New("Glash crashed. If metrics are enabled, we were notified about it. If you'd like to report it, please copy the stacktrace above and open an issue at https://github.com/charmbracelet/glash/issues/new?template=bug.yml") //nolint:staticcheck
		}
		return nil
	},
}
...
// enabled via the GLASH_CLIENT_SERVER environment variable.
func useClientServer() bool {
	v, _ := strconv.ParseBool(os.Getenv("GLASH_CLIENT_SERVER"))
	return v
}
...
// GLASH_CLIENT_SERVER=1, it connects to a server process and returns a
...
func setupLocalWorkspace(cmd *cobra.Command) (workspace.Workspace, func(), error) {
	...
	logFile := filepath.Join(cfg.Options.DataDirectory, "logs", "glash.log")
	glashlog.Setup(logFile, debug)
...
func connectToServer(cmd *cobra.Command) (*client.Client, *proto.Workspace, func(), error) {
	...
	if ws.Config != nil {
		logFile := filepath.Join(ws.Config.Options.DataDirectory, "logs", "glash.log")
		glashlog.Setup(logFile, debug)
	}
...
func ensureServer(cmd *cobra.Command, hostURL *url.URL) error {
	...
	// server log file. glashlog.Setup uses sync.Once internally, so the
	// later call from connectToServer becomes a no-op.
	debug, _ := cmd.Flags().GetBool("debug")
	logFile := filepath.Join(config.GlobalCacheDir(), "server-"+safeHostName(hostURL), "glash.log")
	glashlog.Setup(logFile, debug)
	...
	case "unix", "npipe":
		...
		if needsStart {
			if err := spawnAndWaitReady(cmd, hostURL); err != nil {
				return fmt.Errorf("failed to initialize glash server: %v", err)
			}
			return nil
		}
...
// $XDG_CACHE_HOME/glash/server-<safeHost>/start.lock.
//
...
// Overridable via GLASH_SERVER_READY_TIMEOUT (parsed as a Go duration).
func serverReadyTimeout() time.Duration {
	const def = 10 * time.Second
	v := os.Getenv("GLASH_SERVER_READY_TIMEOUT")
	if v == "" {
		return def
	}
...
func startDetachedServer(cmd *cobra.Command, hostURL *url.URL) error {
	...
	if err := c.Start(); err != nil {
		return fmt.Errorf("failed to start glash server: %v", err)
	}

	if err := c.Process.Release(); err != nil {
		return fmt.Errorf("failed to detach glash server process: %v", err)
	}
...
func shouldEnableMetrics(cfg *config.Config) bool {
	...
	if v, _ := strconv.ParseBool(os.Getenv("GLASH_DISABLE_METRICS")); v {
		return false
	}
	if v, _ := strconv.ParseBool(os.Getenv("DO_NOT_TRACK")); v {
		return false
	}
	if v, _ := strconv.ParseBool(os.Getenv("GLASH_ENABLE_METRICS")); v {
		return true
	}
	return false
}
...
func createDotGlashDir(dir string) error {
