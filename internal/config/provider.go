	...
	providerOnce.Do(func() {
		...
		wg.Go(func() {
			...
			if err != nil {
				...
				errs = append(errs, fmt.Errorf("Glash was unable to fetch an updated list of providers from %s. Consider setting GLASH_DISABLE_PROVIDER_AUTO_UPDATE=1 to use the embedded providers bundled at the time of this Glash release. You can also update providers manually. For more info see glash update-providers --help.\n\nCause: %w", catwalkURL, err)) //nolint:staticcheck
				return
			}
		...
		wg.Go(func() {
			...
			if err != nil {
				errs = append(errs, fmt.Errorf("Glash was unable to fetch updated information from Hyper: %w", err)) //nolint:staticcheck
				return
			}
			hyperProvider = item
			hyperFound = true
