...
func TestReloadFromDisk_CompilesHookMatchers(t *testing.T) {
	...
	workDir := t.TempDir()
	dataDir := t.TempDir()
	configPath := filepath.Join(workDir, "glash.json")
	cfgJSON := `{
        "hooks": {
            "PreToolUse": [
...
// verifies the "^shell$" matcher rejects a non-shell tool while accepting
// shell.
func assertHookFilters(t *testing.T, store *config.ConfigStore) {
	...
	require.NoError(t, err)
	require.Equal(t, 1, match.HookCount, "shell must match ^shell$ matcher")
}
