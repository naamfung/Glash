...
import (
	...
	"github.com/charmbracelet/glash/internal/discover"
...
func PushPopGlashEnv() func() {
	var found []string
	for _, ev := range os.Environ() {
		if strings.HasPrefix(ev, "GLASH_") {
			pair := strings.SplitN(ev, "=", 2)
			if len(pair) != 2 {
				continue
			}
			found = append(found, strings.TrimPrefix(pair[0], "GLASH_"))
		}
	}
	...
	for _, ev := range found {
		os.Setenv(ev, os.Getenv("GLASH_"+ev))
	}
...
func (c *Config) setDefaults(workingDir, dataDir string) {
	...
	if len(c.Options.GlobalContextPaths) == 0 {
		glashConfigDir := filepath.Dir(GlobalConfig())
		c.Options.GlobalContextPaths = []string{
			filepath.Join(glashConfigDir, "GLASH.md"),
			filepath.Join(filepath.Dir(glashConfigDir), "AGENTS.md"),
		}
	}
	...
	if str, ok := os.LookupEnv("GLASH_DISABLE_PROVIDER_AUTO_UPDATE"); ok {
		c.Options.DisableProviderAutoUpdate, _ = strconv.ParseBool(str)
	}

	if str, ok := os.LookupEnv("GLASH_DISABLE_DEFAULT_PROVIDERS"); ok {
...
// so an unrelated glash.json placed above the project is never picked
...
func GlobalConfig() string {
	if glashGlobal := os.Getenv("GLASH_GLOBAL_CONFIG"); glashGlobal != "" {
		return filepath.Join(glashGlobal, fmt.Sprintf("%s.json", appName))
	}
...
func GlobalCacheDir() string {
	if glashCache := os.Getenv("GLASH_CACHE_DIR"); glashCache != "" {
		return glashCache
	}
...
func GlobalConfigData() string {
	if glashData := os.Getenv("GLASH_GLOBAL_DATA"); glashData != "" {
		return filepath.Join(glashData, fmt.Sprintf("%s.json", appName))
	}
	...
	// for windows, it should be in `%LOCALAPPDATA%/glash/`
	// for linux and macOS, it should be in `$HOME/.local/share/glash/`
	if runtime.GOOS == "windows" {
		localAppData := cmp.Or(
			os.Getenv("LOCALAPPDATA"),
			filepath.Join(os.Getenv("USERPROFILE"), "AppData", "Local"),
...
// fallback keeps Glash from silently adopting state files placed above
// the current project.
...
func GlobalSkillsDirs() []string {
	if glashSkills := os.Getenv("GLASH_SKILLS_DIR"); glashSkills != "" {
		return []string{glashSkills}
	}
	...
	// On Windows, also load from app data on top of `$HOME/.config/glash`.
...
// ProjectSkillsDir returns the default project directories for which Glash
