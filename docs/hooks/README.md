...
the agent lifecycle, allowing you to both build on top of Glash, customize
...
- Glash ships with a builtin `glash-hook` skill write, edit, and configure
  hooks; just tell Glash how to configure Glash
- Glash currently supports just one hook, `PreToolUse`, with plans to support
...
our **project-level** `glash.json`. Relative paths like `./no-haskell.sh` work
here because the project root is your working directory. If you're configuring
a global hook (`~/.config/glash/glash.json`), use an absolute path instead.
...
Hooks run through Glash's embedded POSIX shell (`mvdan.cc/sh`) — the same
...
- **Permissive shebang fallback**: if the absolute path in a shebang
  doesn't exist (e.g. `#!/bin/bash` on Windows), Glash falls back to a
...
- **Timeout behavior**: when a hook exceeds its timeout, Glash cancels the
  ...
  yield. If the hook still hasn't returned, Glash abandons it, logs a
...
Hooks can be added to your `glash.json` (or `.glash.json`) at both the global
...
> work fine in project-level `glash.json` because the project root is also
> your working directory. For **global** config (`~/.config/glash/`),
> however, you must use either an absolute path or an inline command:
>
> ```jsonc
> // Global ~/.config/glash/glash.json
> {
>   "hooks": {
>     "PreToolUse": [
>       {
>         "command": "/home/you/.config/glash/hooks/no-haskell.sh"
...
3. Runs all matching hooks **in parallel** through Glash's embedded POSIX
...
5. Applies the result **before** permission checks. If the aggregated decision
   ...
   for it. If it's `allow`, Glash treats that as affirmative pre-approval and
...
however you'll incur some additional overhead as Glash will still parse and
run each hook.
...
| ---------------------------- | ---------------------------------------------- |
| `GLASH`                      | Always `1` when running under Glash.           |
| `AGENT`                      | Always `glash`.                                |
| `AI_AGENT`                   | Always `glash`.                                |
| `GLASH_EVENT`                | The hook event name (e.g. `PreToolUse`).       |
| `GLASH_TOOL_NAME`            | The tool being called (e.g. `bash`).           |
| `GLASH_SESSION_ID`           | Current session ID.                            |
| `GLASH_CWD`                  | Working directory.                             |
| `GLASH_PROJECT_DIR`          | Project root directory.                        |
| `GLASH_TOOL_INPUT_COMMAND`   | For `bash` calls: the shell command being run. |
| `GLASH_TOOL_INPUT_FILE_PATH` | For file tools: the target file path.          |

The `GLASH`, `AGENT`, and `AI_AGENT` markers are also set by the `bash`
tool, so a script can detect "am I running under Glash?" the same way in
either context.
...
Hooks communicate back to Glash via **exit code** and `stdout`/`stderr`. The
...
deterministic based on the order hooks appear in `glash.json`.
...
If a hook exceeds its timeout, Glash cancels its context and treats the
...
```bash
#!/usr/bin/env bash
# Block rm -rf commands in the shell tool. Otherwise stay silent so the
# normal permission flow runs.

if echo "$GLASH_TOOL_INPUT_COMMAND" | grep -qE 'rm\s+-(rf|fr)\s+/'; then
...
returns `decision: "allow"`, which tells Glash to pre-approve the call:
...
if [[ "$GLASH_TOOL_INPUT_FILE_PATH" == *.go ]]; then
...
```jsonc
{ "command": "echo \"$(date -Iseconds) $GLASH_TOOL_NAME\" >> ./tools.log" }
```
...
Glash hooks are broadly compatible with [Claude Code
...
hooks run under Glash unchanged. This document covers the Glash-specific API
only — anything not documented here isn't guaranteed to work.

One intentional divergence: Glash treats `updated_input` as a shallow-merge
...
4. `decision` precedence: `deny` > `allow` > `null`. First deny determines the
   ...
   is `allow`, Glash pre-approves the tool call and skips the permission
