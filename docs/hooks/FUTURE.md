...
contents. Glash tells the agent the files exist and are relevant, and the agent
...
Paths are resolved relative to `GLASH_CWD`. Non-existent paths are dropped with
a debug log (don't fail the hook over a missing file).

### How the agent sees it

Glash appends a short note to the turn's context along the lines of:
...
- Should `context_files` paths be constrained to `GLASH_PROJECT_DIR`? Probably
...
Today Glash supports exactly one hook event, `PreToolUse`. That's enough to gate
