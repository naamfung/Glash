# Glash Development Guide

## Project Overview

Glash is a terminal-based AI coding assistant built in Go by
...
The module path is `github.com/charmbracelet/glash`.
...
internal/
  ...
  config/
    ...
    load.go                        glash.json loading and validation
  ...
  hooks/                           Hook engine: runs user shell commands on hook events
    ...
    input.go                       Stdin payload builder, env vars, stdout parsing (Glash + Claude Code compat)
...
- **Context files**: Glash reads AGENTS.md, GLASH.md, CLAUDE.md, GEMINI.md
...
- **Pub/sub**: `internal/pubsub` for decoupled communication between agent,
  UI, and services.
- **Hooks**: User-defined shell commands in `glash.json` that fire before
